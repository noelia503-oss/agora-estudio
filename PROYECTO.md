...
- **Despliegue:** interfaz web estática empaquetada como aplicación de escritorio con Electron para Mac; el icono inicia Ágora y carga los archivos locales de la propia app, sin Vite, navegador externo ni servidor separado. La PWA para iPhone/iPad necesita alojamiento HTTPS. El repositorio GitHub de origen es privado; el alojamiento y la visibilidad final de la web quedan pendientes de confirmar según el plan disponible. La carpeta `material/`, las preguntas y las copias de seguridad no se publican; IndexedDB sigue siendo local a cada dispositivo. GitHub Pages hace pública la web incluso si el repositorio de origen es privado y, en cuentas gratuitas, exige que el repositorio sea público.
